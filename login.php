...
<body>
  ...
  if (@$_GET['id'] != "sinup" && @$_GET['id'] != "forgot" && @$_GET['id'] != "changep") {
    if (isset($_POST['b1'])) {
      $t1 = $_POST['t1'];
      $t2 = $_POST['p1'];
      $sql = "SELECT * FROM `login` WHERE Email='$t1' AND password=MD5('$t2')";
      $s = mysqli_query($link, $sql);
      $nos = mysqli_num_rows($s);

      if ($nos > 0) {
        $_SESSION['t1'] = $t1;
        header("location:account.php?home=1");
  ...
  if (@$_GET['id'] == "sinup") {
    if (isset($_POST['bf1'])) {
      ...
      $email = $_POST['t1'];
      $password = $_POST['t7'];
      $sql = " INSERT INTO `teachers`(`name`, `mobile`, `Email`, `Branch`) VALUES ('$fname','$mobile','$email','$branch')";
      $a = mysqli_query($link, $sql);
      $log = "INSERT INTO `login`(`Email`, `password`, `mobile`)  VALUES ('$email',MD5('$password'),'$mobile')";
    ...
    <div class="container"><div class="card   shadow-lg mx-auto"  style="width:40%" id="topvalue">
      ...
      <input type="Email" name="t1" id="t1"  required autofocus class="form-control">
      </div>
      <div class="form-label-group">  
  ...
  if (@$_GET['id'] == 'forgot') {
    ...
    if (isset($_POST['bf1'])) {
      ...
      $t2 = $_POST['p1'];
      //  $t3=$_POST['t3'];
      $sql = "SELECT * FROM `login` WHERE  Email='$id' AND mobile='$t2'";
      $s = mysqli_query($link, $sql);
      $nos = mysqli_num_rows($s);
      if ($nos > 0) {
        $_SESSION['fuser'] = $id;
...
<button type="submit" class="btn bt-md btn-primary btn-block text-uppercase" name="bf1" id="bf1">Submit</button>
  ...
  if (@$_GET['id'] == 'changep') {
    ...
    if (isset($_POST['bc1'])) {
      ...
      $t3 = $_POST['t1'];
      $con = "UPDATE `login` SET `password`=MD5('$t3') WHERE  `Email`='$user'";
